...
def parse_03005_type_501(data: bytes):
    ...
    for i in range(150):
        ...
        temps_150.append(str(t))

    temps_pt = []
    for i in range(150):
        start = 332 + i * 2
        t = int.from_bytes(data_real[start:start + 2], byteorder='big', signed=True) * 0.001
        temps_pt.append(f"{t:.3f}")
